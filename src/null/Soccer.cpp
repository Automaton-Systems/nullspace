...
inline static void SimulateAxis(Powerball& ball, Map& map, u32* pos, s16* vel) {
  u32 previous = *pos;
...
static inline bool CanPickupBall(Soccer& soccer, Player& player, Powerball& ball) {
  ...
  if (soccer.connection.settings.DisableBallThroughWalls) {
    Vector2f direction = soccer.GetBallPosition(soccer.player_manager, ball, GetMicrosecondTick()) - player.position;
...
static bool OnMode3(const Vector2f& position, u32 frequency) {
...
static bool OnMode5(const Vector2f& position, u32 frequency) {
